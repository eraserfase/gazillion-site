...
A tape stop effect slows audio to a halt so its pitch and speed fall together, like a tape machine or turntable losing power. To make one, apply a speed ramp that drops to zero over a short time (usually a quarter note to a bar), shape it so it slows gradually and then fast, and duck the volume at the very end. You can automate a pitch-and-speed change, use your DAW's built-in slow-down, or use a tape stop plugin.
...
- **Curve**: slow at first, then faster
...
- Curve the ramp so it starts gently and drops off fast at the end.
...
Use a region fade set to Slow Down on the end of the region, and adjust the fade length and curve to control how quickly it stops.
